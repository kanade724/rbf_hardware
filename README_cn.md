...
统一 GUI 左侧是带网格和中心参考线的手写板。手写板会随窗口可用空间等比放大或缩小，已绘制的轨迹、网格和采样点会同步重绘。内部始终使用固定逻辑坐标，因此调整窗口大小不会改变最终 16 维数据。程序从鼠标轨迹中等距选择 8 个点，将每个点的 x、y 坐标组成 16 个数，并缩放到 0～100。
...
固定文件名便于硬件端持续读取；每次写入都会覆盖上一次内容，因此该表只表示
最新处理的一个手写数字。
...
4. 不同数字绝不累计到同一张实验表；
5. 一次补处理多行历史数据时，每处理一行都会覆盖该表，最终保留最后一行；
