...
from .config import load_config, resolve_device, resolve_dtype, resolve_paths
from .data import HardwareFeatureScaler, audit_dataset_isolation, load_dataset_bundle
...
def run_training(config_path: Path) -> TrainingResult:
    ...
    try:
        ...
        logger.info(
            "[AUDIT] Model selection, scaler fitting, and classifier fitting consume train_out only; test_out is used for final prediction/metrics only",
        )
