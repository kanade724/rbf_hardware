"""Training and evaluation dataset loading."""
...
class DatasetBundle:
    ...
    train_reference_features: np.ndarray | None
    test_reference_features: np.ndarray | None
...
def _validate_reference(
    ...
    label_column = int(config["data"].get("label_column", -1))
    features = np.delete(reference[: len(split.labels)], label_column, axis=1).astype(
        np.float32,
        copy=False,
    )
    return int(len(reference)), features
...
def load_dataset_bundle(paths: Any, config: dict[str, Any]) -> DatasetBundle:
    ...
    train_reference_features: np.ndarray | None = None
    test_reference_features: np.ndarray | None = None
    if bool(config["data"].get("require_reference_label_match", True)):
        train_reference_rows, train_reference_features = _validate_reference(
            train,
            paths.train_reference_file,
            config,
        )
        test_reference_rows, test_reference_features = _validate_reference(
            test,
            paths.test_reference_file,
            config,
        )
    ...
    return DatasetBundle(
        ...
        test_reference_features=test_reference_features,
    )
...
def audit_dataset_isolation(bundle: DatasetBundle, paths: Any, config: dict[str, Any]) -> dict[str, Any]:
    ...
    reference_enabled = bool(config["data"].get("require_reference_label_match", True))
    hashes = {
    ...
    return {
        ...
        "reference_labels_verified": reference_enabled,
